  ...
  try {
    ...
    for (size_t i = 0; i < tokens.size(); ++i) {
      ...
      if (kind == BRACE) {
        ...
        ConfigTree::addChildSetValue(tokens, &i, layers_[depth_ + 2],
                                     layers_[depth_ + 1]);
      } else if (kind == LOCATION || (kind >= LISTEN && kind <= RETURN)) {
        ConfigTree::addChildSetValue(tokens, &i, layers_[depth_ + 1],
                                     layers_[depth_]);
      } else {
...
void ConfigTree::deleteTree_(ConfigNode* node) {
  if (!node) return;
  for (std::vector<ConfigNode*>::iterator it = node->getChildren().begin();

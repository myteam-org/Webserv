...
TEST(ReadingRequestBodyStateTest, SwitchesToLengthStateWhenContentLengthPresent) {
    ...
    http::ReadingRequestBodyState* state = new http::ReadingRequestBodyState(http::kContentLength, cfg);
...
// === TEST 2 ===
// Transfer-Encoding: chunked がある場合、ChunkedState に切り替わるか（モックが必要なため簡易検証）
...
//     http::ReadingRequestBodyState* state = new http::ReadingRequestBodyState(http::kChunked, cfg);
...
TEST(ReadingRequestBodyStateTest, ReturnsErrorIfNoValidBodyEncoding) {
    ...
    // ボディ不要 → NULL が返る
    http::IState* next = ctx.createReadingBodyState(headers);
